...
// Globals
//
uni_hal_spi_context_t* g_uni_hal_spi_ctx[6] = { 0 };
    ...
    if (ctx != nullptr && !uni_hal_spi_is_inited(ctx)) {
        ...
        if (instance != nullptr) {
            ...
            if (ctx->config.pin_nss != nullptr) {
                ...
                ctx->config.pin_nss->gpio_type = ctx->config.nss_hard ? UNI_HAL_GPIO_TYPE_ALTERNATE_PP : UNI_HAL_GPIO_TYPE_OUT_PP;
            ...
            if(ctx->config.dma_tx != nullptr) {
                ...
                LL_DMA_SetPeriphRequest(module, stream, LL_DMAMUX1_REQ_SPI4_TX);
            ...
            SPI_InitStruct.NSS = (ctx->config.nss_hard && ctx->config.pin_nss)? LL_SPI_NSS_HARD_OUTPUT : LL_SPI_NSS_SOFT;
            ...
            if (result != false) {
                ...
                if (ctx->config.pin_nss != nullptr) {
                   ...
                   LL_SPI_SetNSSPolarity(instance, LL_SPI_NSS_POLARITY_HIGH);
                } else {
...
bool uni_hal_spi_transmit_async(uni_hal_spi_context_t *ctx, const uint8_t *data, uint32_t len) {
    ...
    if (uni_hal_spi_is_inited(ctx) && data && len > 0U) {
        ...
        LL_DMA_ConfigAddresses(module, stream, (uint32_t) data, LL_SPI_DMA_GetTxRegAddr(instance),
